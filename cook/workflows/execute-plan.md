...
@~/.claude/cook/references/git-integration.md
</required_reading>
...
2. Use Task tool with subagent_type="cook-executor" and model="{executor_model}":
...
2. For each segment in order:
   ...
   B. If routing = Subagent:
      ```
      Spawn Task tool with subagent_type="cook-executor" and model="{executor_model}":
...
See `~/.claude/cook/references/tdd.md` for TDD plan structure.
</tdd_plan_execution>
...
See ~/.claude/cook/references/checkpoints.md for complete checkpoint guidance.
</step>
...
Create `.planning/phases/XX-name/{phase}-USER-SETUP.md` using template from `~/.claude/cook/templates/user-setup.md`.
...
Use ~/.claude/cook/templates/summary.md for structure.
...
**Also available:**
- `/cook:verify-work {phase}-{plan}` — manual acceptance testing before continuing
...
- `/cook:discuss-phase {Z+1}` — gather context first
...
**Also available:**
- `/cook:verify-work` — manual acceptance testing before completing milestone
- `/cook:add-phase <description>` — add another phase before completing
