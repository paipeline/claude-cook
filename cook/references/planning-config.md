...
"git": {
  ...
  "phase_branch_template": "cook/phase-{phase}-{slug}",
  "milestone_branch_template": "cook/{milestone}-{slug}"
}
```
...
| `git.phase_branch_template` | `"cook/phase-{phase}-{slug}"` | Branch template for phase strategy |
| `git.milestone_branch_template` | `"cook/{milestone}-{slug}"` | Branch template for milestone strategy |
</config_schema>
...
**Note:** Most COOK operations use direct file reads or explicit paths, which work regardless of gitignore status.
...
- Branch name from `phase_branch_template` (e.g., `cook/phase-03-authentication`)
...
- Branch name from `milestone_branch_template` (e.g., `cook/v1.0-mvp`)
