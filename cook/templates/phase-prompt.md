...
> **Note:** Planning methodology is in `agents/cook-planner.md`.
...
<!-- For checkpoint task examples and patterns, see @~/.claude/cook/references/checkpoints.md -->
...
**Wave is pre-computed:** Wave numbers are assigned during `/cook:plan-phase`. Execute-phase reads `wave` directly from frontmatter and groups plans by wave number. No runtime dependency analysis needed.
...
See `~/.claude/cook/templates/user-setup.md` for full schema and examples
...
See `~/.claude/cook/workflows/verify-phase.md` for verification logic.
