...
@~/.claude/cook/workflows/pm-check.md
...
```
No PM configuration found. Run /cook:pm-start to initialize.
```
Exit with code 0.
...
Spawn cook-planner agent to create plans for this phase.

```
Spawning planner for Phase {X}: {name}...
```

Use Task tool to spawn cook-planner:
- Provide phase goal from ROADMAP.md
- Provide PROJECT.md context
- Wait for plans to be written

Then spawn cook-plan-checker to verify plan quality.
...
Run /cook:complete-milestone to archive and plan next.
```
