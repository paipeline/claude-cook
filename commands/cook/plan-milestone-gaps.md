...
<objective>
Create all phases necessary to close gaps identified by `/cook:audit-milestone`.

Reads MILESTONE-AUDIT.md, groups gaps into logical phases, creates phase entries in ROADMAP.md, and offers to plan each phase.

One command creates all fix phases — no manual `/cook:add-phase` per gap.
</objective>

<execution_context>
<!-- Spawns cook-planner agent which has all planning expertise baked in -->
</execution_context>
...
```
No audit gaps found. Run `/cook:audit-milestone` first.
```
...
**Also available:**
- `/cook:execute-phase {N}` — if plans already exist
...
`/cook:audit-milestone` — re-audit to verify gaps closed
`/cook:complete-milestone {version}` — archive when audit passes
```
...
- [ ] Changes committed
- [ ] User knows to run `/cook:plan-phase` next
</success_criteria>
